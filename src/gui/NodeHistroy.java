...
//		frame.getContentPane().add(panel_4);
	...
	*/
	 ...
	 private  JPanel createVoltageChart() {
	     ...
	     ResultSet rs = null;
	 	//ceate a dataset
			TimeSeries timeseries = new TimeSeries("voltage", org.jfree.data.time.Second.class);
			NodeTime nodetime=new NodeTime();
		try {
			stm=conn.createStatement();
			rs = stm.executeQuery(sql);
...
private  JPanel createHumidityChart() {
	     ...
	     ResultSet rs = null;
	 	//ceate a dataset
			TimeSeries timeseries = new TimeSeries("humidity", org.jfree.data.time.Second.class);
			NodeTime nodetime=new NodeTime();
		try {
			stm=conn.createStatement();
			rs = stm.executeQuery(sql);
...
private  JPanel createvibrationChart() {
	     ...
	     ResultSet rs = null;
	 	//ceate a dataset
			TimeSeries timeseries = new TimeSeries("vibration", org.jfree.data.time.Second.class);
			NodeTime nodetime=new NodeTime();
		try {
			stm=conn.createStatement();
			rs = stm.executeQuery(sql);
